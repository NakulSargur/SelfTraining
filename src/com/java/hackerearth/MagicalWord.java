...
public class MagicalWord {
		...
		try {
			...
			for (int i = 0; i < noOfTestCases; i++) {
				...
				for (int j = 0; j < strLength; j++) {
					char magicalChar = getMagicalCharecter((int) strValue.charAt(j), 0);
					if (magicalChar != ' ') {
						outputValue.append(magicalChar);
					}
				}
	...
	private static char getMagicalCharecter(int inputNumber, int moveIndex) {
		int testNumber = inputNumber + moveIndex;
		if (!isPrimeNumber(testNumber)) {
			if (moveIndex > 0) {
				return getMagicalCharecter(inputNumber + moveIndex, moveIndex - 1);
			} else {
				return getMagicalCharecter(inputNumber - moveIndex, moveIndex + 1);
			}
		}
		return (char) testNumber;
	}
